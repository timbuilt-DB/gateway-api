...
const envelopeSchemaJson = JSON.parse(readFileSync(join(__dirname, '../src/schemas/envelope.schema.json'), 'utf-8'));
const jobtreadQuerySchemaJson = JSON.parse(readFileSync(join(__dirname, '../src/schemas/jobtread.query.schema.json'), 'utf-8'));
const jobtreadPushJobToQboSchemaJson = JSON.parse(readFileSync(join(__dirname, '../src/schemas/jobtread.pushJobToQbo.schema.json'), 'utf-8'));
const makeTriggerSchemaJson = JSON.parse(readFileSync(join(__dirname, '../src/schemas/make.trigger.schema.json'), 'utf-8'));
// Compile schemas
...
app.get('/admin/logs', async (request, reply) => {
    try {
        ...
        const { traceId, brand, action, status } = request.query;
        const filters = {};
        if (traceId)
